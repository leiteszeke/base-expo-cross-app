...
export const initAnalytics = async (user: User, mixpanelRef: MutableRefObject<MixPanel>) => {
...
export const logEvent = (eventName: string, eventData?: Generic) => {
  MyEvents.emit(EventKey.LogEvent, { eventName, eventData })
}

export const logScreenView = async (screenName: string) => {
  MyEvents.emit(EventKey.ScreenView, screenName)
}
...
export const clearMixpanel = (mixpanelRef: MutableRefObject<ExpoMixpanelAnalytics>) => {
  if (Config.useMixpanelAnalytics && mixpanelRef.current) {
